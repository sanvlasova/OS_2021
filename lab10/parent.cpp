...
static void child_callback (EV_P_ ev_child *w, int revents) {
  ...
  cout << "подпроцесс вернул код:" << WEXITSTATUS(w->rstatus));
...
int main (int argc, char *argv[]) {
  cout << "parent начала работу" << endl;
  //printf("parent начала работу\n");
  ...
  cout << "parent ждет нажатия клавиши" << endl;
  //printf("parent программа ждет нажатия клавиши\n");

  ev_io_init(&stdin_watcher, stdin_callback, STDIN_FILENO, EV_READ);
  ev_io_start(EV_DEFAULT_ &stdin_watcher);
  ev_run (EV_DEFAULT_ 0);
  
  cout << "parent завершил работу" << endl;
