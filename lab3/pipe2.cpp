...
static void* f1(void* arg){ //функция потока 1
      ...
      while (*(bool*)arg == 0){ //пока флаг заверш.потока 1 не установлен
            ...
            rv = write(filedes[1], &buf, sizeof(int)); //записать сообщение из буфера в неименованный канал
            if (rv > 0) {
                 ...
                 cout << "Не удалось записать сообщение. Канал перполнен";
                 sleep(1);
                 }
...
int main(){ //основная программа
      ...
      pipe2(filedes, O_NONBLOCK); // Создание неименованного канала
